...
Compare every component that exists in BOTH the project and the toolkit. Reports IN_SYNC / TOOLKIT_NEWER / PROJECT_NEWER / DIVERGED for each, plus cross-reference staleness. Read-only.

## Installation

```
cp -r claude-toolkit-diff/ <project>/.claude/skills/
```

Invoke as `/claude-toolkit-diff`.
